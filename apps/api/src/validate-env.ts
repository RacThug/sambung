 ...
 * misconfigured deployment refuses to start (fail-fast) rather than boot
 * degraded - the same boot seam as load-env and the TRUST_PROXY env-gate live
 * on.
 *
 ...
 *
 * 4. PAYMENT_GATEWAY must not be `fake` (#167) - a process that confirms
 *    bookings against fabricated payment events.
 *
 * WHEN THEY RUN: not "when NODE_ENV says production", but whenever this process
 * cannot prove it is a local sandbox - see `deployment-env.ts` for the rule and
 * why it is derived from the browser-facing origins rather than a declared mode
 * (#193). Dev and every suite declare only loopback origins, so they are
 * untouched.
 *
...
export function validateEnv(env: NodeJS.ProcessEnv): void {
  const evidence = deploymentEvidence(env);
  if (evidence === null) return;

  // Appended to every refusal: a guard that fires for an unexplained reason is
  // a guard people work around. This names the variable that convinced it and
  // the one way back.
  const because =
  ...
  //
  // On the Garage-on-VPS shape it does NOT fail - it really rewrites a live

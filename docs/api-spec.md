...
| Unknown keys | Every request **body and query schema** in `packages/shared` **rejects unknown keys** - built with `strictObject`, so a misspelled field (`{galleryCapp}`, `?form=`) is a **400** naming it, never a `200`/default that silently ignored it (ADR-0031). **Responses stay lenient** (plain `z.object`): a client must tolerate fields a newer server adds. A route that takes **no** body says so and refuses one: the **verb-subresources** (`archive`, `unarchive`, `cancel`, `dismiss`, `handle`, `pay`, `sync`, plus `auth/refresh` + `auth/logout`) and every **DELETE** carry `@NoBody()`, so `POST /bookings/:id/cancel {"refund":"full"}` is the same 400, not a 200 that ignored it (#152). Two things sit outside all this, by construction rather than omission: the **payment webhook** (plucks a few fields from a large provider payload) and the **iCal feed** (hand-rolled parser, not a zod object), both parsed in `apps/api`. |
...
`MeResponse = { user, tenant, memberships }`. `401` without/with a garbage token, or when the token names a membership that no longer exists - deliberately *unlike* `refresh`, which falls back to another seat. `me` describes the token it was given; `refresh` mints a new one. The two compose as 401 → refresh → retry.
...
> **On settings.** This section previously read "settings, billing, invites → 403". That is now precise rather than narrowed: the settings **write** is owner-only, while `GET /settings` stays open to any signed-in user - decided in #67 ([ADR-0030](adr/0030-a-cap-is-a-preference-the-ceiling-is-the-guard.md)) because the property workbench needs the gallery cap to know when a gallery is full, and reaffirmed here. "Billing" names nothing that exists yet.
...
Fields: `name` (required, 2-160), `address?`, `latitude?`/`longitude?` (valid ranges), `description?`, `licenseNo?` (NIB), `timeZone?` (the property's local clock - one of `Asia/Jakarta` | `Asia/Makassar` | `Asia/Jayapura`, defaulting to WITA; #145, [ADR-0028](adr/0028-property-local-is-a-column-not-an-assumption.md)). `timeZone` is always present on `PropertyResponse` and accepted on PATCH; an unknown zone is a `400` at the boundary and the `property_time_zone_known` CHECK backstops it. It is owner-facing only - the public payload (§4.7) does not carry it, because the funnel has no use for it. Response includes derived `verified: boolean` - true iff `licenseNo` is non-empty (FR-PROP-3). A public page needs ≥1 photo + ≥1 unit with a price **above zero** to render "complete" (FR-PROP-1 AC) - the API exposes `publishable: boolean` computed from that rule. A zero-rupiah unit is storable (§4.6) but never counts toward publishability: it's a placeholder, not a sellable listing. An **archived** unit (or any unit under an archived property) also never counts - `publishable` is `isSellable AND active`, so a property whose only priced unit is archived reports `publishable: false` (§4.8, [ADR-0005](adr/0005-archived-is-derived-not-cascaded.md)). `PropertyResponse` and `UnitResponse` carry `archivedAt: string | null` (the owner sees their own retired inventory); the public payload never does.

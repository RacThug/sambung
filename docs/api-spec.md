...
| Unknown keys | Request **bodies and query params reject unknown keys** - built with `strictObject` in `packages/shared`, so a misspelled field (`{galleryCapp}`, `?form=`) is a **400** naming it, never a `200`/default that silently ignored it (ADR-0031). **Responses stay lenient** (plain `z.object`): a client must tolerate fields a newer server adds. The one inbound exception is the provider payment webhook, which plucks a few fields from a large provider payload and so is parsed leniently in `apps/api`, off this convention. |

...
| Unknown keys | Every request **body and query schema** in `packages/shared` **rejects unknown keys** - built with `strictObject`, so a misspelled field (`{galleryCapp}`, `?form=`) is a **400** naming it, never a `200`/default that silently ignored it (ADR-0031). **Responses stay lenient** (plain `z.object`): a client must tolerate fields a newer server adds. Three things sit outside this, by construction rather than omission: the **payment webhook** (plucks a few fields from a large provider payload) and the **iCal feed** (hand-rolled parser, not a zod object), both parsed in `apps/api`; and the **verb-subresource POSTs** (`archive`, `unarchive`, `cancel`, `dismiss`, `handle`, `pay`, `sync`, plus `auth/refresh` + `auth/logout`), which declare no body at all, so a stray key sent to one is still ignored (a known gap, tracked as #152). |

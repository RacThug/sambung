...
| Quote | CTA replaced by "online payment not available yet - contact the owner" when the Tenant has no gateway | `onlinePaymentsAvailable` | `publicPropertyResponseSchema` | `GET /public/properties/:slug` | BE (derived: a payment credential exists) | [code] |
...
| Picker: available | Green line + nights + total + Book CTA. *(REQ-PA-04: when `onlinePaymentsAvailable` is false, the quote still renders - availability stays honest - but the CTA is the contact-the-owner message; ADR-0039 decision 4.)* |

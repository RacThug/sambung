...
**Built** (#38, migration 0012, [ADR-0027](adr/0027-dismiss-is-a-judgement-resolve-is-a-measurement.md)). Three things about the shipped table are worth knowing, because two of them depart from the sketch above:

1. **It is the one honest exception to derive-don't-store.** Availability, `publishable` and effective-archived are all derived because the rows exist to derive them from. A conflict is a row that *failed to exist* - the losing VEVENT is in no table, and the feed is transient. You cannot query for what was refused.
2. **No `raw_vevent` column, and no `stay` daterange.** The parser deliberately drops `SUMMARY`/`DESCRIPTION` so imported guest PII never enters (ADR-0025); storing the raw block would re-admit it through a side door. The refused stay is `check_in`/`check_out` date columns like every other stay - the schema's only `daterange` is built inline inside the exclusion constraint, and a second idiom for one fact is where drift starts.
3. **`status` is `open | resolved | dismissed`, and re-detection treats the two closed states differently.** `resolved` is a *measurement* the next sync may re-take (the clash healed, then came back → reopen); `dismissed` is the owner's *judgement*, which no cron may undo - otherwise a dismissed item resurrects every 30 minutes. Auto-closing is guarded by "healthy feed with ≥ 1 event", the same rule as the absent-UID cancellation.

Standard tenant furniture: `tenant_id` with the `(unit_id, tenant_id)` composite FK of §4.5 (load-bearing, since the writer is the RLS-bypassed cron), an RLS `tenant_isolation` policy for the owner-side read, and `unique (channel_connection_id, external_uid)` for idempotent re-detection. The connection FK is `on delete cascade` - unlike `booking`'s `set null`, because a booking is ledger while a conflict is a todo about a feed.

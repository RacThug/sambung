...
| UX-03a | WHEN the fleet line reports a problem, it shall still name the age of the oldest good check whenever one exists. "Unreachable for ten minutes" and "unreachable for three days" are not the same emergency, and a warning that hides which understates the damage - the FR-05 rule, applied to the fleet. | web `calendar-page.test.tsx` (stale fleet names "last checked 6 hours ago"; erroring fleet names "last good check 3 days ago") |

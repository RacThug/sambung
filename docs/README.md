...
| [`page-spec.md`](./page-spec.md) | …adding or changing a page/route | Every SPA page: purpose, route + URL state, endpoints consumed, states. **`status: legacy`** - it documents pages already built; new work starts in `pages/` |
| [`pages/`](./pages/README.md) | …building a **new** page, or specifying one | Per-page specs, written **before** building a page (unlike `page-spec.md`, which documents pages already built): the template every spec follows, plus the shared dashboard list pattern a spec only writes deltas against |
| [`spec/`](./spec/) | …building a product-phase requirement (P0/P1/P2), before writing code | One EARS spec per requirement, driven by [`prd-product.md`](./prd-product.md): one testable sentence per row, each naming the test that verifies it. A row with no test is a claim, not a requirement |
...
| [`research/`](./research/) | …deciding a number that depends on how a third party behaves | Dated findings against outside sources, each claim labelled primary or secondary, ending in what it means for us and what would change the answer. Facts about other people's systems go stale - re-check the date before trusting one |

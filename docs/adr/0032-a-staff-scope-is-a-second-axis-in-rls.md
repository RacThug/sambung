...
**Revoking access is immediate for data, not for the token.** Removing an assignment takes effect on the next query, because RLS reads the table on every statement rather than trusting anything baked into the JWT. Removing the *user* leaves their access token cryptographically valid until it expires (≤15 min), but every scoped read then returns nothing and no refresh can mint another - the honest limit of a stateless token, and the reason the scope was never put in the token in the first place. **(Amended 2026-07-22 by [ADR-0034](0034-one-identity-many-memberships.md): removal now ends a *membership* rather than deleting the account, so an account with a second seat DOES refresh - into that other seat. The guarantee narrows to "no refresh can mint a token for a tenant the account no longer holds a membership at", which is what this paragraph was actually relying on.)**

...
**One email address, one account, across all of Sambung.** `app_user.email` is globally unique because login is `email + password` with no tenant in the request - two rows sharing an address would make "which account is this?" unanswerable. So the same person cannot be staff at two Tenants with one address. Per-tenant emails would mean a tenant selector at login, which is a product change, not a schema tweak - filed as a follow-up.

That limitation is checked **at invite time, globally**, and the first draft got this wrong: the pre-check was tenant-scoped (`emailInUse`, under RLS) while the constraint it stands in for is global, so inviting someone who already had an account elsewhere created and *emailed* a link that answered `409 email_taken` on every attempt, forever, with nothing to tell either party why. Independent review found it. The check now runs on the owner connection - the only one that can see across tenants - and refuses before anything is created or sent. Asking globally leaks nothing new: `POST /auth/register` already answers "does this address have an account?" to anyone at all, unauthenticated.

**An expired invite must not block its own replacement.** `staff_invite_live_email_uniq` is partial on `accepted_at is null and revoked_at is null` and **cannot** also test `expires_at > now()` - an index predicate must be immutable. So a lapsed invite went on occupying the one live slot for its address, and re-inviting answered `409 invite_already_pending` forever. Also found in review. `create` now closes stale rows for that address first (`supersedeExpiredInvites`), and "pending" everywhere means *live*, expiry included. Marking the stale row `revoked_at` is bookkeeping, not a claim: `refusalReason` checks expiry **before** revocation, so the holder of the stale link is still told it expired rather than that someone withdrew it. The order there is load-bearing and tested.

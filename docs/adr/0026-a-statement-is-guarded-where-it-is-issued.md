...
- `guardSession` wraps the transaction's `session.prepareQuery` in place. Every query shape (select/insert/update/delete, raw `execute`, `count`) funnels through `prepareQuery` at execution (await) time, so one guard there catches a deferred builder when it finally runs - build site or not.
...
- **This couples to two Drizzle internals** - that a transaction's `session` is the executor, and that `prepareQuery` is the sole funnel. That coupling is deliberate and *licensed by a test*: the escape spec builds a query inside `run` and awaits it after, asserting it throws. If a Drizzle upgrade ever routes execution around `prepareQuery`, that test goes red - the isolation cannot silently reopen. An internal you can reach is safe only when a tripwire watches it; here one does.
